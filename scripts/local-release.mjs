...
const packages = [
	...
	{ directory: "packages/coding-agent", name: "millrace-cli" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "millrace-cli-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-")
		? `millrace-cli-${platform}.zip`
		: `millrace-cli-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createMillraceCliShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "millrace-cli.cmd"))) {
			writeFileSync(
				join(installDirectory, "millrace-cli.cmd"),
				'@ECHO off\r\n"%~dp0node_modules\\.bin\\millrace-cli.cmd" %*\r\n',
			);
			writeFileSync(
				join(installDirectory, "millrace-cli.ps1"),
				'& "$PSScriptRoot/node_modules/.bin/millrace-cli.ps1" @args\n',
			);
			return;
		}
		writeFileSync(
			join(installDirectory, "millrace-cli.cmd"),
			'@ECHO off\r\n"%~dp0node_modules\\.bin\\millrace-cli.exe" %*\r\n',
		);
		writeFileSync(
			join(installDirectory, "millrace-cli.ps1"),
			'& "$PSScriptRoot/node_modules/.bin/millrace-cli.exe" @args\n',
		);
		return;
	}
	symlinkSync(join("node_modules", ".bin", "millrace-cli"), join(installDirectory, "millrace-cli"));
}
...
if (rootPackageJson.name !== "millrace-cli-monorepo") {
...
if (!options.skipInstall) {
	...
	createMillraceCliShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createMillraceCliShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(
		`  ${join(outDir, `millrace-cli-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`,
	);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(
		`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "millrace-cli.exe" : "millrace-cli")} --help`,
	);
	...
	console.log(
		`  ${join(nodeInstallDirectory, process.platform === "win32" ? "millrace-cli.cmd" : "millrace-cli")} --help`,
	);
	...
	if (!options.skipBunInstall) {
		...
		console.log(
			`  ${join(bunInstallDirectory, process.platform === "win32" ? "millrace-cli.cmd" : "millrace-cli")} --help`,
		);
	}
}
