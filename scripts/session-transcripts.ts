 ...
 *   --analyze      Spawn millrace-cli subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("millrace-cli", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn millrace-cli: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".millrace-cli/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn millrace-cli subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".millrace-cli/agent/AGENTS.md");
