...
const agentDirEnvName = "MILLRACE_CLI_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "MILLRACE_CLI_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific MILLRACE_CLI_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force MILLRACE_CLI_OFFLINE=1 / MILLRACE_CLI_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.MILLRACE_CLI_OFFLINE = "1";
		env.MILLRACE_CLI_SKIP_VERSION_CHECK = "1";
		env.PI_OFFLINE = "1";
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir
		? mkdtempSync(join(tmpdir(), "millrace-cli-startup-benchmark-"))
		: undefined;
