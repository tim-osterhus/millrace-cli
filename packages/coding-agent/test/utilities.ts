...
// ============================================================================
// OAuth API key resolution from ~/.millrace-cli/agent/auth.json
// ============================================================================
...
/**
 * Resolve API key for a provider from ~/.millrace-cli/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.millrace-cli/agent/auth.json
 */
...
/** Path to the real Millrace CLI agent config directory */
export const MILLRACE_CLI_AGENT_DIR = getAgentDir();

/**
 * Get an AuthStorage instance backed by ~/.millrace-cli/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `millrace-cli-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
