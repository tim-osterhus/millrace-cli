...
async function capturePackageCommandHelp(command: "install" | "remove"): Promise<string> {
	const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
	try {
		const handled = await handlePackageCommand([command, "--help"]);
...
describe("package command help", () => {
	const legacyConfigDir = ".pi";

	it("should describe project-local install settings with the configured project config directory", async () => {
		const output = await capturePackageCommandHelp("install");

		expect(output).toContain(`Install project-locally (${CONFIG_DIR_NAME}/settings.json)`);
		expect(output).not.toContain(`${legacyConfigDir}/settings.json`);
	});

	it("should describe project-local remove settings with the configured project config directory", async () => {
		const output = await capturePackageCommandHelp("remove");

		expect(output).toContain(`Remove from project settings (${CONFIG_DIR_NAME}/settings.json)`);
		expect(output).not.toContain(`${legacyConfigDir}/settings.json`);
	});
});
...
Content`,
		...
		it("should auto-discover root markdown skills from Millrace CLI skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to the project config dir", async () => {
...
Content`,
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user Millrace CLI skills", async () => {
		...
		it("should use the project config dir as baseDir for project skills", async () => {
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".millrace-cli", "agent");
		...
		it("should dedupe user skill entries when the user agent skills dir is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to project config auto-discovery", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.millrace-cli\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should install user npm packages into the Millrace CLI-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to the project config settings base", () => {
