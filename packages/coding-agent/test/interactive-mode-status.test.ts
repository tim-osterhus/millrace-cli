...
"[Extensions]
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		const home = homedir();
		const cwd = path.join(home, "Development", "millrace-cli");
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			contextFiles: [
				{ path: path.join(home, ".millrace-cli", "agent", "AGENTS.md") },
		...
		expect(output).toContain("~/.millrace-cli/agent/AGENTS.md, AGENTS.md");
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.chatContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.millrace-cli/agent/AGENTS.md");
		expect(output).toContain("~/Development/millrace-cli/AGENTS.md");
		expect(output).not.toContain("~/.millrace-cli/agent/AGENTS.md, AGENTS.md");
	});
