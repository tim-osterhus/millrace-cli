...
export async function main(args: string[], options?: MainOptions) {
	...
	const startupBenchmark =
		isTruthyEnvFlag(process.env.MILLRACE_CLI_STARTUP_BENCHMARK) || isTruthyEnvFlag(process.env.PI_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: MILLRACE_CLI_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
