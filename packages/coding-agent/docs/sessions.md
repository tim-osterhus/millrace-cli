...
Millrace CLI saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.millrace-cli/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
millrace-cli -c                  # Continue most recent session
millrace-cli -r                  # Browse and select from past sessions
millrace-cli --no-session        # Ephemeral mode; do not save
millrace-cli --session <path|id> # Use a specific session file or partial session ID
millrace-cli --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `millrace-cli -r` opens the same picker at startup.
...
When available, Millrace CLI uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `millrace-cli -r`.
...
When `/tree` switches away from one branch to another, Millrace CLI can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
