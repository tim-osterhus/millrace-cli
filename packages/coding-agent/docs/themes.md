> Millrace CLI can create themes. Ask it to build one for your setup.
...
Millrace CLI loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.millrace-cli/agent/themes/*.json`
- Project: `.millrace-cli/themes/*.json`
...
On first run, Millrace CLI detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.millrace-cli/agent/themes
vim ~/.millrace-cli/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, Millrace CLI reloads it automatically for immediate visual feedback.
...
Millrace CLI uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Millrace CLI falls back to the nearest approximation.
